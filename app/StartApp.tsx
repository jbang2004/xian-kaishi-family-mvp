...
export function StartApp() {
  ...
  const backgroundReminderStatus = notificationPermission === "granted"
    ? backgroundReminder ? "切到其他页面时会尝试系统提醒；锁屏或省电模式可能延迟" : "已获得系统权限，需要时可以在这里开启"
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {screen === "plan" && <div className="screen plan-screen">
        ...
        <div className={`plan-list ${!stages.length ? "is-empty" : ""}`}>{!stages.length && <button ref={addNodeButtonRef} type="button" className="empty-plan empty-plan-action" onClick={addStage} aria-label="增加第一个时间节点"><Mascot mood="breathe" compact /><strong>先加一件容易开始的小事</strong><span className="empty-plan-cta"><b aria-hidden="true">＋</b>增加第一个节点</span><small>任务、休息和家庭活动都可以</small></button>}{stages.map((stage, index) => { const expanded = editingStageId === stage.id; const hasIssue = planStageIssueIds.has(stage.id); const issueText = stageIssueById.get(stage.id); const titleInvalid = Boolean(planItemErrors[index]?.title); const timeInvalid = Boolean(planItemErrors[index]?.time); const stageName = stage.title.trim() || "未命名事项"; return <div data-stage-id={stage.id} className={`stage-editor ${expanded ? "is-expanded" : "is-collapsed"} ${timeInvalid ? "has-stage-issue" : titleInvalid ? "needs-title" : ""} ${stage.status === "tomorrow" ? "muted-stage" : ""}`} key={stage.id}>
          <button className="stage-summary" aria-expanded={expanded} aria-controls={`stage-editor-${stage.id}`} aria-label={`${expanded ? "收起" : "编辑"}第${index + 1}项${stageName}`} onClick={() => setEditingStageId(expanded ? "" : stage.id)}><span className="stage-summary-icon"><AppIcon name={stage.icon} /></span><span className="stage-summary-copy"><strong>{stageName}</strong><small>{titleInvalid ? "还没写下这件事" : timeInvalid ? "需要调整时间" : `${formatPlanClock(stage.start, data.planStart, data.planEnd)}—${formatPlanClock(stage.end, data.planStart, data.planEnd)} · ${stage.kind === "rest" ? "休息放松" : effortCopy[stage.effort]}`}</small></span><span className={`stage-summary-energy ${stage.energy ? "" : "is-zero"}`}><b>{stage.energy || "—"}</b><small>{stage.energy ? "能量" : "不计"}</small></span><i aria-hidden="true">⌄</i></button>
          {expanded && <div id={`stage-editor-${stage.id}`} className="stage-editor-body"><button className="stage-icon-button" onClick={() => { setEditingStageId(stage.id); go("icon-picker"); }} aria-label={`更换${stageName}图标`}><AppIcon name={stage.icon} /><small>换图标</small></button><div className="stage-main"><input data-stage-title className="stage-title-input" aria-label={`第${index + 1}项名称`} aria-invalid={titleInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} placeholder="例如：阅读、吃饭或休息" maxLength={24} autoComplete="off" spellCheck={false} enterKeyHint="done" value={stage.title} onChange={e => updateStage(stage.id, { title: e.target.value })} onKeyDown={e => { if (e.key === "Enter" && !e.nativeEvent.isComposing) { e.preventDefault(); if (!e.currentTarget.value.trim()) { setToast("先写下这件事，再继续安排"); return; } e.currentTarget.blur(); focusStageDetails(stage.id); } }} /><div className="time-range"><input aria-label={`${stage.title || `第${index + 1}项`}开始时间`} aria-invalid={timeInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} type="time" value={stage.start} onFocus={() => beginStageTimeEdit(stage.id, "start")} onBlur={e => endStageTimeEdit(stage.id, "start", e.currentTarget.value)} onChange={e => updateStageStart(stage.id, e.target.value)} /><span>—</span><input aria-label={`${stage.title || `第${index + 1}项`}结束时间`} aria-invalid={timeInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} type="time" value={stage.end} onFocus={() => beginStageTimeEdit(stage.id, "end")} onBlur={e => endStageTimeEdit(stage.id, "end", e.currentTarget.value)} onChange={e => updateStageEnd(stage.id, e.target.value)} /></div><div className="stage-meta"><button className={`effort-pill effort-${stage.kind === "rest" ? "rest" : stage.effort}`} onClick={() => { setEditingStageId(stage.id); go("effort"); }}>{stage.kind === "rest" ? "休息放松" : effortCopy[stage.effort]} · 调整</button><div className={`task-energy ${stage.energy ? "" : "is-zero"}`}><span><b>共同商量能量</b><strong>{stage.energy ? `${stage.energy} 点` : "不计能量"}</strong></span><input className="task-energy-range branded-range" type="range" min="0" max="5" step="1" value={stage.energy} aria-label={`${stageName}完成后的家庭能量`} aria-valuetext={stageEnergyLabel(stage.energy)} style={{ "--range-progress": `${stage.energy * 20}%` } as CSSProperties} onChange={e => updateStage(stage.id, { energy: Number(e.target.value) })} /><div className="task-energy-scale" aria-label={`${stageName}能量快捷选择`}>{[0,1,2,3,4,5].map(value => <button type="button" key={value} className={stage.energy === value ? "selected" : ""} aria-pressed={stage.energy === value} aria-label={`${stageName}${value === 0 ? "不计能量" : `${value}点能量`}`} onClick={() => updateStage(stage.id, { energy: value })}>{value === 0 ? "不计" : value}</button>)}</div><small className="task-energy-note">可以拖动，也可以直接点选；休息事项可以选“不计”。</small></div></div>{issueText && <p id={`stage-issue-${stage.id}`} className="stage-inline-issue" aria-live="polite">{issueText}</p>}</div><div className="stage-actions"><button onClick={() => moveStage(index, -1)} disabled={index === 0} aria-label="向上移动">↑</button><button onClick={() => moveStage(index, 1)} disabled={index === stages.length - 1} aria-label="向下移动">↓</button><button onClick={() => removeStage(stage, index)} aria-label={`删除${stageName}，可撤销`}>×</button></div></div>}
        </div>; })}</div>
      ...
      {screen === "confirm" && <div className="screen confirm-screen">
        ...
        {plannedStartOffset !== 0 && <div className={`confirm-timing-note ${plannedStartPassed ? "is-late" : "is-early"}`} role="status"><AppIcon name="alarm" /><span><strong>{plannedStartPassed ? `原定 ${plannedStartLabel} 已过；现在开始会整体顺延` : `如果现在开始，时间会比原定 ${plannedStartLabel} 整体前移`}</strong><small>每一项保留原时长和休息空档；从现在开始，最晚 {formatPlanClock(shiftedPlanEndLabel, startNowLabel, shiftedPlanEndLabel)} 温和收尾。</small></span></div>}
      ...
      {screen === "night-saved" && lastSavedSession && <div className="screen night-saved-screen">
        <div className="saved-hero"><div><span className="eyebrow">已安全保存在家庭日历</span><h1>今晚的合作，<br />已经留下来了</h1><p>这不是成绩，也不要求连续打卡。</p></div><Mascot mood="celebrate" compact /></div>
