...
export function StartApp() {
  ...
  const startRestNow = () => {
    ...
    setAdjustments(value => value + 1); setToast(`现在休息10分钟，事项预计${scheduledEndTime(result.items, result.planEnd)}结束`); go("running");
  };
  ...
  return <main className={`site-shell ${motionReduced ? "reduce-motion" : ""}`} data-screen={screen}>
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {screen === "plan" && <div className="screen plan-screen">
        ...
        <div className={`plan-list ${!stages.length ? "is-empty" : ""}`}>{!stages.length && <button ref={addNodeButtonRef} type="button" className="empty-plan empty-plan-action" onClick={addStage} aria-label="增加第一个时间节点"><Mascot mood="breathe" compact /><strong>先加一件容易开始的小事</strong><span className="empty-plan-cta"><b aria-hidden="true">＋</b>增加第一个节点</span><small>任务、休息和家庭活动都可以</small></button>}{stages.map((stage, index) => { const expanded = editingStageId === stage.id; const hasIssue = planStageIssueIds.has(stage.id); const issueText = stageIssueById.get(stage.id); const titleInvalid = Boolean(planItemErrors[index]?.title); const timeInvalid = Boolean(planItemErrors[index]?.time); const stageName = stage.title.trim() || "未命名事项"; return <div data-stage-id={stage.id} className={`stage-editor ${expanded ? "is-expanded" : "is-collapsed"} ${timeInvalid ? "has-stage-issue" : titleInvalid ? "needs-title" : ""} ${stage.status === "tomorrow" ? "muted-stage" : ""}`} key={stage.id}>
          ...
          {expanded && <div id={`stage-editor-${stage.id}`} className="stage-editor-body">
            ...
            <div className="stage-main">
              {titleInvalid && <div id={`stage-title-hint-${stage.id}`} className="stage-edit-hint"><b>先写名称</b><span>时间与 1 点能量已先放好，随后都能调整</span></div>}
              ...
              <div className="stage-meta">
                <button className={`effort-pill effort-${stage.kind === "rest" ? "rest" : stage.effort}`} aria-label={`调整节点类型和用力程度：${stage.kind === "rest" ? "休息放松" : `要做的事，${effortCopy[stage.effort]}`}`} onClick={() => { setEditingStageId(stage.id); go("effort"); }}>{stage.kind === "rest" ? "休息放松 · 调整" : `任务 · ${effortCopy[stage.effort]}`}</button>
                <button type="button" className={`energy-detail-toggle ${stage.energy ? "" : "is-zero"}`} aria-label={`调整${stageName}完成后的家庭能量：${stageEnergyLabel(stage.energy)}`} aria-expanded={editingEnergyStageId === stage.id} aria-controls={`stage-energy-${stage.id}`} onClick={() => setEditingEnergyStageId(current => current === stage.id ? "" : stage.id)}><span>能量</span><strong>{stage.energy ? `${stage.energy} 点` : "不计"}</strong><i aria-hidden="true">⌄</i></button>
            ...
            <div className="stage-actions"><button onClick={() => moveStage(index, -1)} disabled={index === 0} aria-label="向上移动">↑</button><button onClick={() => moveStage(index, 1)} disabled={index === stages.length - 1} aria-label="向下移动">↓</button><button onClick={() => removeStage(stage, index)} aria-label={`删除${stageName}，可撤销`}>×</button></div>
          </div>}
        </div>; })}</div>
        ...
        <div id="plan-node-guidance" className="gentle-note">{stages.length >= MAX_PLAN_STAGES ? "可以合并相近事项，或删除不需要的一项；已有内容不会自动删掉。" : `今晚安排可以随时调整。当前 ${stages.length}/${MAX_PLAN_STAGES} 个节点，留一点空白更容易开始。`}</div>
      </div>}
      ...
      {screen === "running" && <div className={`screen running-screen ${stageDue ? "has-due-decision" : ""}`}>
        ...
        <div className={`next-stage-preview ${softLanding ? "is-landing" : ""}`}><span><small>{softLanding ? "下一步先看一眼" : "这一段之后"}</small><strong>{nextPendingStage ? nextPendingStage.title : "就可以温和收尾"}</strong></span>{nextPendingStage && <time>{formatPlanClock(nextPendingStage.start, data.planStart, data.planEnd)}</time>}</div>
      ...
      {screen === "wrap" && <div className="screen wrap-screen">
        ...
        <div className="wrap-action-dock"><button className="primary-button" onClick={finishNight}>保存记录并结束今晚</button>{hasDeferredStages && <button className="secondary-button wrap-resume-button" onClick={resumeTonightFromWrap}>还想继续今晚</button>}<small>{promptReflection ? settlementFooter : `催促感可以不填 · ${settlementFooter}`}</small></div>
      </div>}
      ...
      {screen === "review" && <div className="screen with-nav review-screen">
        ...
        <div ref={calendarDetailRef} key={selectedDay} className="day-detail">
          <div className="day-detail-header" role="status" aria-live="polite" aria-atomic="true"><div><small>家庭夜晚</small><strong>{selectedDateLabel}</strong></div>{selectedSessionSummary && <span>{selectedSessionSummary.settlements} 次收尾</span>}</div>
          {!selectedSessions.length && !selectedRewards.length ? <div className="empty-day"><Mascot mood="breathe" compact /><span><strong>这一天没有留下记录</strong><small>也没关系，日历不要求每天使用。</small></span></div> : <>
            {selectedSessionSummary && <section className="daily-summary"><div className="daily-summary-title"><AppIcon name="moon" /><div><strong>{selectedSessionSummary.settlements > 1 ? `这一晚分${selectedSessionSummary.settlements}次留下记录` : selectedSessionSummary.completed ? `这一晚完成了${selectedSessionSummary.completed}个${selectedSessionSummary.completionUnit === "tasks" ? "事项" : "节点"}` : "这一晚选择了温和收尾"}</strong><small>先看整体，不用逐条比较每一次。</small></div></div><div className="daily-summary-stats"><span><b>{selectedSessionSummary.completed}</b><small>{selectedSessionSummary.completionUnit === "tasks" ? "完成事项" : "完成节点"}</small></span><span><b>{selectedSessionSummary.adjustments}</b><small>主动调整</small></span><span><b>+{selectedSessionSummary.energy}</b><small>家庭能量</small></span></div><div className="daily-summary-note"><strong>{selectedSessionSummary.reflection ? promptReflectionCopy[selectedSessionSummary.reflection] : "催促感还没有记录"}</strong><span>{selectedSessionSummary.stageTitles.length ? `这一晚做过：${selectedSessionSummary.stageTitles.join("、")}${selectedSessionSummary.hiddenStageTitleCount ? `，另有 ${selectedSessionSummary.hiddenStageTitleCount} 项` : ""}` : "没有完成事项也可以收尾；记录不会评价孩子。"}</span></div></section>}
        ...
        <section className="one-change-card"><div className="one-change-heading"><span><AppIcon name={reviewSuggestion.icon} /></span><div><small>{weeklyNights ? "规则建议 · 不评价孩子" : "第一次可以从这里开始"}</small><h2>{weeklyNights ? "接下来只试一个小变化" : "先留下一晚真实感受"}</h2></div></div><strong className="one-change-text">{reviewSuggestion.text}</strong><p>{reviewSuggestion.evidence}</p><div className="rule-transparency"><b>{weeklyNights ? "建议怎么来的" : "为什么这样建议"}</b><span>{weeklyNights ? "只使用本周收尾次数、主动调整和大人的催促感记录；不分析孩子身份或能力。" : "还没有本周记录，所以这里只提供一个低压力的起点；不会分析孩子身份或能力。"}</span></div><button aria-pressed={currentWeekFocus?.text === reviewSuggestion.text} className={currentWeekFocus?.text === reviewSuggestion.text ? "saved" : ""} onClick={toggleWeeklyFocus}>{currentWeekFocus?.text === reviewSuggestion.text ? "已放到首页 · 点击移除" : weeklyNights ? "这周就试这一件" : "把这个起点放到首页"}</button></section>
      </div>}
