...
export function StartApp() {
  ...
  const [shiftedPlanUndo, setShiftedPlanUndo] = useState<{ stages: Stage[]; message: string } | null>(null);
  ...
  const updateStageEnd = (id: string, end: string) => {
    ...
    if (delta && hasFollowing) {
      ...
      setShiftedPlanUndo({ stages, message: delta > 0 ? `本项延长 ${delta} 分钟，后续时间已顺延` : `本项缩短 ${Math.abs(delta)} 分钟，后续时间已提前` });
    }
    setStages(shiftFollowingForEndChange(stages, index, end));
  };
  const undoPlanShift = () => {
    if (!shiftedPlanUndo) return;
    setStages(shiftedPlanUndo.stages); setShiftedPlanUndo(null); setToast("已恢复调整前的时间");
  };
