...
export function StartApp() {
  ...
  const planningAhead = clockMinutesUntil(startNowLabel, plannedStartLabel) > 10;
  ...
  return <main className={`site-shell ${motionReduced ? "reduce-motion" : ""}`} data-screen={screen}>
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {screen === "plan" && <div className="screen plan-screen">
        ...
        <div className={`plan-list ${!stages.length ? "is-empty" : ""}`}>{!stages.length && <div className="empty-plan"><Mascot mood="breathe" compact /><strong>今晚还没有节点</strong><span>先加一件最容易开始的小事就好</span></div>}{stages.map((stage, index) => { const expanded = editingStageId === stage.id; const hasIssue = planStageIssueIds.has(stage.id); const issueText = stageIssueById.get(stage.id); const titleInvalid = Boolean(planItemErrors[index]?.title); const timeInvalid = Boolean(planItemErrors[index]?.time); const stageName = stage.title.trim() || "未命名事项"; return <div data-stage-id={stage.id} className={`stage-editor ${expanded ? "is-expanded" : "is-collapsed"} ${timeInvalid ? "has-stage-issue" : titleInvalid ? "needs-title" : ""} ${stage.status === "tomorrow" ? "muted-stage" : ""}`} key={stage.id}>
          ...
          {expanded && <div id={`stage-editor-${stage.id}`} className="stage-editor-body"><button className="stage-icon-button" onClick={() => { setEditingStageId(stage.id); go("icon-picker"); }} aria-label={`更换${stageName}图标`}><AppIcon name={stage.icon} /><small>换图标</small></button><div className="stage-main"><input data-stage-title className="stage-title-input" aria-label={`第${index + 1}项名称`} aria-invalid={titleInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} placeholder="例如：阅读、吃饭或休息" maxLength={24} autoComplete="off" spellCheck={false} enterKeyHint="done" value={stage.title} onChange={e => updateStage(stage.id, { title: e.target.value })} onKeyDown={e => { if (e.key === "Enter" && !e.nativeEvent.isComposing) { e.preventDefault(); if (!e.currentTarget.value.trim()) { setToast("先写下这件事，再继续安排"); return; } e.currentTarget.blur(); setEditingStageId(""); } }} /><div className="time-range"><input aria-label={`${stage.title || `第${index + 1}项`}开始时间`} aria-invalid={timeInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} type="time" value={stage.start} onFocus={() => beginStageTimeEdit(stage.id, "start")} onBlur={e => endStageTimeEdit(stage.id, "start", e.currentTarget.value)} onChange={e => updateStageStart(stage.id, e.target.value)} /><span>—</span><input aria-label={`${stage.title || `第${index + 1}项`}结束时间`} aria-invalid={timeInvalid} aria-describedby={hasIssue ? `stage-issue-${stage.id}` : undefined} type="time" value={stage.end} onFocus={() => beginStageTimeEdit(stage.id, "end")} onBlur={e => endStageTimeEdit(stage.id, "end", e.currentTarget.value)} onChange={e => updateStageEnd(stage.id, e.target.value)} /></div><div className="stage-meta"><button className={`effort-pill effort-${stage.kind === "rest" ? "rest" : stage.effort}`} onClick={() => { setEditingStageId(stage.id); go("effort"); }}>{stage.kind === "rest" ? "休息放松" : effortCopy[stage.effort]} · 调整</button><label className={`task-energy ${stage.energy ? "" : "is-zero"}`}><span><b>共同商量能量</b><strong>{stage.energy ? `${stage.energy} 点` : "不计能量"}</strong></span><input className="task-energy-range branded-range" type="range" min="0" max="5" step="1" value={stage.energy} aria-label={`${stageName}完成后的家庭能量`} aria-valuetext={stageEnergyLabel(stage.energy)} style={{ "--range-progress": `${stage.energy * 20}%` } as CSSProperties} onChange={e => updateStage(stage.id, { energy: Number(e.target.value) })} /><span className="task-energy-scale" aria-hidden="true">{[0,1,2,3,4,5].map(value => <i key={value}>{value === 0 ? "不计" : value}</i>)}</span><small className="task-energy-note">休息或不想用积分交换的事项，可以选“不计”。</small></label></div>{issueText && <p id={`stage-issue-${stage.id}`} className="stage-inline-issue" aria-live="polite">{issueText}</p>}</div><div className="stage-actions"><button onClick={() => moveStage(index, -1)} disabled={index === 0} aria-label="向上移动">↑</button><button onClick={() => moveStage(index, 1)} disabled={index === stages.length - 1} aria-label="向下移动">↓</button><button onClick={() => removeStage(stage, index)} aria-label={`删除${stageName}，可撤销`}>×</button></div></div>}
        </div>; })}</div>
        ...
        <div id="plan-node-guidance" className="gentle-note">{stages.length >= MAX_PLAN_STAGES ? "可以合并相近事项，或删除不需要的一项；已有内容不会自动删掉。" : `今晚安排可以随时调整。当前 ${stages.length}/${MAX_PLAN_STAGES} 个节点，留一点空白更容易开始。`}</div><button className="primary-button" disabled={!stages.length || planHasErrors} onClick={openConfirmPlan}>{planHasErrors ? planErrorPrompt : "下一步：一起确认"}</button>
      </div>}
