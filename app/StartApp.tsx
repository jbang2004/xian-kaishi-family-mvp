          ...
          {expanded && <div id={`stage-editor-${stage.id}`} className="stage-editor-body">
            ...
            <div className="stage-main">
              ...
              <div className="stage-meta">
                <button className={`effort-pill effort-${stage.kind === "rest" ? "rest" : stage.effort}`} aria-label={`调整节点类型和用力程度：${stage.kind === "rest" ? "休息放松" : `要做的事，${effortCopy[stage.effort]}`}`} onClick={() => { setEditingStageId(stage.id); go("effort"); }}>{stage.kind === "rest" ? "休息放松 · 调整" : `要做的事 · ${effortCopy[stage.effort]}`}</button>
                <button type="button" className={`energy-detail-toggle ${stage.energy ? "" : "is-zero"}`} aria-expanded={editingEnergyStageId === stage.id} aria-controls={`stage-energy-${stage.id}`} onClick={() => setEditingEnergyStageId(current => current === stage.id ? "" : stage.id)}><span>完成能量</span><strong>{stage.energy ? `${stage.energy} 点` : "不计"}</strong><i aria-hidden="true">⌄</i></button>
