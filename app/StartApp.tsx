...
function Chevron({ direction = "down", className = "" }: { direction?: "down" | "up" | "left" | "right"; className?: string }) {
...
function Header({ title, home, step }: { title?: string; home?: () => void; step?: string }) {
  return <header className="app-header">
    {home ? <button className="icon-button home-header-button" onClick={home} aria-label="返回首页"><HomeMark /></button> : <span className="header-spacer" />}
...
export function StartApp() {
  ...
  const goHome = () => go(data.consent ? "home" : "welcome");
  ...
  return <main className={`site-shell ${motionReduced ? "reduce-motion" : ""}`} data-screen={screen}>
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {appReady && screen === "welcome" && <div className="screen welcome-screen">
        ...
        <div className="privacy-card welcome-boundary"><div><span className="big-icon"><AppIcon name="privacy" /></span><span><strong>孩子不会被监控或公开比较</strong><small>仅使用家庭化名；不收集学校、年级、位置、录音或社交平台数据。</small></span></div><button type="button" onClick={() => openPrivacy("welcome")}>查看数据保存与删除说明 <Chevron direction="right" /></button></div>
      ...
      {appReady && screen === "privacy" && <div className="screen privacy-screen">
        <Header home={goHome} title="隐私与数据说明" />
      ...
      {screen === "home" && <div className="screen with-nav home-screen">
        ...
        {currentWeekFocus && <button className="insight-card weekly-focus-home" onClick={() => go("review")}><span className="big-icon"><AppIcon name="home-heart" /></span><div><small>这周只试这一件 · 给大人的提醒</small><strong>{currentWeekFocus.text}</strong></div><Chevron direction="right" /></button>}
        <button className={`insight-card support-entry goal-entry-${goalState}`} onClick={goalState === "empty" ? openRewardSetup : goalState === "ready" ? () => openRewardAchieved() : () => go("energy")}><span className="big-icon"><AppIcon name={goalState === "empty" ? "home-heart" : goalState === "ready" ? data.rewardGoal.icon : "plant"} /></span><div><small>{goalState === "empty" ? "下一份家庭期待" : goalState === "ready" ? "家庭期待已点亮" : "家庭期待"}</small><strong>{goalState === "empty" ? "一起定下想共度的家庭时光" : data.rewardGoal.title}</strong><small>{goalState === "empty" ? "从0开始，不用急着定" : goalState === "ready" ? "等你们真的一起实现后再记录" : `还差${progress}点，一起积累`}</small></div><Chevron direction="right" /></button>
      ...
      {screen === "plan" && <div className="screen plan-screen">
        <Header home={goHome} title="一起安排今晚" step="1/3" />
        ...
        <div className="plan-template-shortcuts"><button className="plan-template-entry" onClick={openTemplates}><TemplateIconGroup stages={(data.templates[0] ?? SYSTEM_PLAN_TEMPLATES[0]).stages} limit={3} showRemainder={false} /><span><small>使用模板</small><strong>3 个推荐{data.templates.length ? ` · ${data.templates.length} 个我的` : ""}</strong></span><Chevron direction="right" /></button><button className="plan-template-save" disabled={!stages.length || planHasErrors} onClick={saveCurrentPlanTemplate}>保存为模板</button></div>
        <div className={`plan-list ${!stages.length ? "is-empty" : ""}`}>{!stages.length && <button ref={addNodeButtonRef} type="button" className="empty-plan empty-plan-action" onClick={addStage} aria-label="增加第一个时间节点"><Mascot mood="breathe" compact /><strong>添加今晚第一项</strong><span className="empty-plan-cta"><b aria-hidden="true">＋</b>添加任务或休息</span><small>先定名称、时间和完成能量</small></button>}{stages.map((stage, index) => { const expanded = editingStageId === stage.id; const hasIssue = planStageIssueIds.has(stage.id); const issueText = stageIssueById.get(stage.id); const titleInvalid = Boolean(planItemErrors[index]?.title); const timeInvalid = Boolean(planItemErrors[index]?.time); const stageName = stage.title.trim() || "未命名事项"; return <div data-stage-id={stage.id} className={`stage-editor ${expanded ? "is-expanded" : "is-collapsed"} ${timeInvalid ? "has-stage-issue" : titleInvalid ? "needs-title" : ""} ${stage.status === "tomorrow" ? "muted-stage" : ""}`} key={stage.id}>
          ...
          {expanded && <div id={`stage-editor-${stage.id}`} className="stage-editor-body">
            ...
            <div className="stage-actions">{stages.length > 1 && <><button onClick={() => moveStage(index, -1)} disabled={index === 0} aria-label="向上移动"><Chevron direction="up" /></button><button onClick={() => moveStage(index, 1)} disabled={index === stages.length - 1} aria-label="向下移动"><Chevron direction="down" /></button></>}<button className="stage-delete-button" onClick={() => removeStage(stage, index)} aria-label={`删除${stageName}，可撤销`}>删除</button></div>
          </div>}
        </div>; })}</div>
      ...
      {screen === "templates" && <div className="screen templates-screen">
        ...
        <button className="save-template-callout" disabled={!stages.length || planHasErrors} onClick={saveCurrentPlanTemplate}><span><strong>保存当前安排为模板</strong><small>{stages.length ? `${stages.length} 项 · 以后可以直接套用` : "先回到时间表安排至少一项"}</small></span><b>保存</b></button>
        ...
        <button className="flow-return-action" onClick={() => back("plan")}>返回今晚时间表</button><p className="template-storage-note">个人模板随家庭数据保存；删除家庭数据时会一起删除。</p>
      </div>}

      {screen === "icon-picker" && <div className="screen icon-picker-screen">
        <Header home={goHome} title="选择活动图标" /><span className="eyebrow">{showAllIcons ? `全部 ${ICON_LIBRARY.length} 个图标` : selectedIconOutsideCommon ? `当前图标 + ${commonIconLibrary.length} 个家庭高频图标` : `先显示 ${commonIconLibrary.length} 个家庭高频图标`}</span><h1>这件事看起来像什么？</h1>
        ...
        <button className="icon-library-toggle" aria-expanded={showAllIcons} onClick={() => setShowAllIcons(value => !value)}>{showAllIcons ? "收起到常用图标" : `显示全部 ${ICON_LIBRARY.length} 个图标`}</button><button className="flow-return-action" onClick={() => back("plan")}>不更换，返回时间表</button>
      </div>}

      {screen === "effort" && <div className="screen effort-screen">
        <Header home={goHome} title="调整节点" />
      ...
      {screen === "confirm" && <div className="screen confirm-screen">
        <Header home={goHome} title="共同确认" step="2/3" />
        ...
        <div className={`confirm-action-dock ${planningAhead ? "is-planning-ahead" : ""}`}>{planningAhead ? <><button className="primary-button" onClick={savePlanForLater}>保存时间表，稍后开始</button><button className="confirm-secondary-action" onClick={enterDualStart}>现在开始</button><small>时间表会留在首页；到 {formatPlanClock(plannedStartLabel, data.planStart, data.planEnd)} 再回来一起点亮。</small></> : plannedStartPassed ? <><button className="primary-button" onClick={enterDualStart}>从现在一起开始</button><button className="confirm-secondary-action" onClick={() => go("plan")}>调整时间</button><small>原定 {formatPlanClock(plannedStartLabel, data.planStart, data.planEnd)} 已过；整晚会按原有时长顺延。</small></> : plannedStartOffset > 0 ? <><button className="primary-button" onClick={enterDualStart}>现在一起开始</button><button className="confirm-secondary-action" onClick={savePlanForLater}>按原时间保存</button><small>现在开始会整体前移；也可以等到 {formatPlanClock(plannedStartLabel, data.planStart, data.planEnd)}。</small></> : <><button className="primary-button" onClick={enterDualStart}>两个人一起确认开始</button><button className="confirm-secondary-action" onClick={savePlanForLater}>稍后开始</button><small>下一步，两个人各点一下自己的名字。</small></>}</div>
      </div>}

      {screen === "dual-start" && <div className="screen dual-start-screen">
        <Header home={goHome} title="一起确认" step="3/3" /><div className="dual-start-hero" data-ready={bothParticipantsReady}><div><span className="eyebrow">今晚从这里开始</span><h1>一起确认，<br />然后出发</h1><p>大人和孩子各轻点一次自己的名字。</p></div><Mascot mood={bothParticipantsReady ? "celebrate" : "ready"} compact /></div>
      ...
      {screen === "running" && <div className={`screen running-screen ${stageDue ? "has-due-decision" : ""}`}>
        <Header home={goHome} title="今晚进行中" /><div className="running-hero"><span className="eyebrow">当前阶段 · {activeTonightOrdinal}/{tonightStageCount}</span><Mascot mood={stageDue ? "confirm" : softLanding ? "support" : "breathe"} compact /></div>
        {stageDue && <span className="sr-only" role="status">{activeStage.title}预计到时间了，可以完成、继续或调整。</span>}
        <div className={`active-stage-card ${stageDue ? "is-due" : softLanding ? "is-landing" : ""}`}><AppIcon name={activeStage.icon} /><div><small>计划时间 {formatPlanClock(activeStage.start, data.planStart, data.planEnd)}—{formatPlanClock(activeStage.end, data.planStart, data.planEnd)}</small><h1>{activeStage.title}</h1><span className={`effort-pill effort-${activeStage.effort}`}>{activeStage.kind === "rest" ? "休息放松" : effortCopy[activeStage.effort]}</span><span className={`active-energy ${activeStage.energy ? "" : "is-zero"}`}>{activeStage.energy ? `完成后 +${activeStage.energy} 能量` : "这一项不计能量"}</span></div><div className="stage-timer"><small>{stageDue ? "现在确认下一步" : softLanding ? "还有1分钟，准备收尾" : "距离到点提醒"}</small><strong>{stageDue ? "到时间了" : gentleRemainingLabel(remainingSeconds)}</strong><div className="time-progress" role="progressbar" aria-label={`${activeStage.title}剩余时间`} aria-valuemin={0} aria-valuemax={activeStageSeconds} aria-valuenow={Math.min(activeStageSeconds, remainingSeconds)} aria-valuetext={stageDue ? "预计到时间，可以完成、继续或调整" : gentleRemainingLabel(remainingSeconds)}><i style={{ width: `${Math.max(0, Math.min(100, remainingSeconds / activeStageSeconds * 100))}%` }} /></div></div></div>
        {!stageDue && <><div className={`next-stage-preview ${softLanding ? "is-landing" : ""}`}><span><small>{softLanding ? "下一步先看一眼" : activeStage.kind === "rest" && nextPendingStage ? `休息后继续 · ${nextPendingDuration}分钟` : "这一段之后"}</small><strong>{nextPendingStage ? nextPendingStage.title : "完成今晚计划"}</strong></span>{nextPendingStage && <time>{formatPlanClock(nextPendingStage.start, data.planStart, data.planEnd)}</time>}</div>
        <details className="timeline-disclosure"><summary><span><small>今晚进度 · 事项预计 {formatPlanClock(liveScheduledEnd, data.planStart, data.planEnd)} 结束</small><strong>{completedStageCount}/{tonightStageCount} 个阶段已完成</strong></span><b>查看全部 <Chevron /></b></summary><div className="mini-timeline">{stages.map((stage, index) => <div key={stage.id} className={`${stage.status} ${index === activeIndex ? "now" : ""}`}><i /><span>{stage.title}</span><small>{stage.status === "done" ? "完成" : stage.status === "tomorrow" ? "明天" : formatPlanClock(stage.start, data.planStart, data.planEnd)}</small></div>)}</div></details></>}
      ...
      {screen === "transition" && <div className={`screen transition-screen ${transitionReason}-transition`}>
        <Header home={goHome} title={transitionReason === "completed" ? "阶段完成" : "阶段提醒"} /><div className="transition-hero"><div><span className="eyebrow">{transitionReason === "completed" ? "这一段完成了" : "到点提醒"}</span><h1>{transitionReason === "completed" ? `${activeStage.title}告一段落` : `${activeStage.title}预计到时间了`}</h1><p className="lead">{transitionReason === "completed" ? "接下来可以继续、休息或调整。" : "选择完成、继续，或调整计划。"}</p></div><div className="transition-art"><AppIcon name={transitionReason === "completed" ? "check" : "moon"} /><Mascot mood={transitionReason === "completed" ? "celebrate" : "confirm"} compact /></div></div>
        ...
        <div className="transition-actions"><button className="primary-button" onClick={continueToNext}>{hasNextPending ? `进入${nextPendingStage?.title ?? "下一阶段"}` : "进入今晚收尾"}</button><div><button className="secondary-button" onClick={extendCurrent}>{canStartRest ? (transitionReason === "completed" ? "继续 10 分钟" : "再继续 10 分钟") : "再休息 10 分钟"}</button>{canStartRest && <button className="soft-button" onClick={startRestNow}>休息 10 分钟</button>}</div><button className="text-button" onClick={openAdjust}>调整计划</button></div>
      </div>}

      {screen === "adjust" && <div className="screen adjust-screen">
        <Header home={goHome} title="调整今晚" /><div className="title-with-mascot"><div><span className="eyebrow">先看变化，再确认</span><h1>现在更适合怎么调整？</h1></div><Mascot mood="support" compact /></div>
      ...
      {screen === "wrap" && <div className="screen wrap-screen">
        <Header home={goHome} title="今晚收尾" /><div className="wrap-hero"><div><span className="eyebrow">共同确认 · 30 秒</span><h1>完成今晚计划</h1><p>确认完成情况，保存今晚的记录和能量。</p></div><Mascot mood="celebrate" compact /></div>
      ...
      {screen === "night-saved" && lastSavedSession && <div className="screen night-saved-screen">
        <Header home={goHome} title="今晚记录" /><div className="saved-hero"><div><span className="eyebrow">已存入家庭日历</span><h1>今晚已保存</h1><p>完成情况和能量都已记录。</p></div><Mascot mood="celebrate" compact /></div>
      ...
      {screen === "reward-setup" && <div className="screen reward-setup-screen">
        <Header home={goHome} title="家庭期待" step="一起商量" />
      ...
      {screen === "reward-saved" && lastRedeemedReward && <div className="screen reward-saved-screen">
        <Header home={goHome} title="家庭期待" /><div className="reward-saved-hero"><div><span className="eyebrow">已保存在家庭日历</span><h1>这份期待，<br />已经实现</h1><p>下一轮从0开始，过去的家庭时光仍然留在这里。</p></div><Mascot mood="celebrate" compact /></div>
      ...
      {screen === "review" && <div className="screen with-nav review-screen">
        <Header home={goHome} title="家庭日历" /><span className="eyebrow">记录过的夜晚会留在这里</span><div className="month-nav"><button onClick={() => shiftMonth(-1)} aria-label="上个月"><Chevron direction="left" /></button><h1>{calendarYear}年{calendarMonth + 1}月</h1><button onClick={() => shiftMonth(1)} aria-label="下个月"><Chevron direction="right" /></button></div>
        ...
        <div id="calendar-day-detail" ref={calendarDetailRef} key={selectedDay} className="day-detail">
          ...
          {!selectedSessions.length && !selectedRewards.length ? <div className="empty-day"><Mascot mood="breathe" compact /><span><strong>这一天没有留下记录</strong><small>也没关系，日历不要求每天使用。</small></span></div> : <>
            ...
            {selectedSessions.length > 0 && <button className="day-details-toggle" aria-expanded={dayDetailsExpanded} aria-controls="day-session-details" onClick={() => { setSessionDeleteArmedId(""); setDayDetailsExpanded(value => !value); }}><span>{dayDetailsExpanded ? "收起单次明细" : `查看${selectedSessions.length}次收尾明细`}</span><Chevron /></button>}
      ...
      {screen === "settings" && <div className="screen with-nav settings-screen">
        <Header home={goHome} title="设置" /><div className="settings-group"><h2>家庭称呼</h2><div className="setting-row"><span>孩子化名</span><strong>{data.childAlias}</strong></div><div className="setting-row"><span>大人称呼</span><strong>{data.guardianAlias}</strong></div><button className="setting-action" onClick={() => openProfile("settings")}>修改家庭称呼 <Chevron direction="right" /></button></div>
        ...
        <div className="settings-group"><h2>隐私与数据</h2><div className="setting-row"><span>未收集年级和学校</span><strong>已启用</strong></div><div className="setting-row"><span>数据状态</span><strong>{syncLabel}</strong></div>{pendingCloudDeletion && <div className="pending-delete-note" role="status"><AppIcon name="alarm" /><span><strong>云端副本等待清理</strong><small>只暂存随机家庭令牌；联网后自动重试，不包含孩子资料。</small></span></div>}<button className="setting-action" onClick={() => openPrivacy("settings")}>查看隐私与数据说明 <Chevron direction="right" /></button><button className="setting-action" onClick={exportData}>导出家庭数据 <Chevron direction="right" /></button><button className="setting-action danger" disabled={deletingData} onClick={requestDeleteData}>{deletingData ? "正在删除本机与云端数据…" : "删除全部家庭数据"} {!deletingData && <Chevron direction="right" />}</button></div>
        <button className="risk-entry" onClick={() => go("risk")}><AppIcon name="privacy" /><div><strong>有些情况，需要更多支持</strong><small>查看风险提示与转介建议</small></div><Chevron direction="right" /></button>
      </div>}

      {screen === "risk" && <div className="screen risk-screen">
        <Header home={goHome} title="支持边界" />
