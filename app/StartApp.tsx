...
export function StartApp() {
  ...
  const startShiftVerb = plannedStartPassed ? "顺延" : "前移";
  const shiftedPlanEndLabel = addMinutes(startNowLabel, Math.max(1, durationMinutes(data.planStart, data.planEnd)));
  ...
  return <main className={`site-shell ${motionReduced ? "reduce-motion" : ""}`} data-screen={screen}>
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {screen === "adjust" && <div className="screen adjust-screen">
        ...
        <div className={`adjust-decision-dock ${effectiveAdjustChoice ? "is-ready" : "is-waiting"}`} aria-label="调整预览与确认"><div className={`change-preview ${effectiveAdjustChoice ? "" : "is-waiting"}`} role="status" aria-live="polite" aria-atomic="true"><small>{effectiveAdjustChoice ? "本次调整预览" : "先一起选一种方式"}</small><strong>{effectiveAdjustChoice === "extend" ? `${activeStage.title}${canStartRest ? "延长" : "再休息"}10分钟，最晚${addMinutes(data.planEnd, 10)}收尾` : effectiveAdjustChoice === "rest" ? activeStageCompleted ? `从现在休息10分钟，之后进入${nextPendingStage?.title ?? "今晚收尾"}，约${restPlanPreview.planEnd}前收尾` : `从现在休息10分钟，之后只继续剩余时长，约${restPlanPreview.planEnd}前收尾` : effectiveAdjustChoice === "swap" ? "调换后两项，时长和休息空档保持不变" : effectiveAdjustChoice === "tomorrow" ? "把下一项移到明天" : effectiveAdjustChoice === "finish" ? "保留已完成的部分，今晚温和收尾" : "选择后先看清变化，再由双方确认；现在还不会改动时间表。"}</strong></div><p className="adjust-safety-note">确认前不会改动 · 调整不扣能量 · 已完成进展会保留</p><div className="adjust-dock-actions"><button className="primary-button" disabled={!effectiveAdjustChoice} onClick={applyAdjustment}>{!effectiveAdjustChoice ? "先选择一种调整" : effectiveAdjustChoice === "finish" ? "确认并温和收尾" : "双方确认调整"}</button><button className="adjust-cancel-button" onClick={returnFromAdjust}>取消</button></div></div>
      </div>}
      ...
      {screen === "reward-achieved" && <div className="screen achievement-screen">
        ...
        {rewardReachedFromNight && lastSavedSession && <div className="achievement-night-saved"><AppIcon name="check" /><span><strong>今晚的收尾已经先保存</strong><small>{lastSavedSession.completedCount ? `${lastSavedSession.completedCount} 个完成阶段` : "温和收尾"} · +{lastSavedSession.energyEarned} 家庭能量 · 日历可回看</small></span></div>}
