  ...
  return <main className={`site-shell ${motionReduced ? "reduce-motion" : ""}`} data-screen={screen}>
    ...
    <section className={`phone-shell ${isOnline ? "" : "is-offline"}`} ref={phoneShellRef} aria-hidden={deleteArmed || undefined} inert={deleteArmed || undefined}>
      ...
      {screen === "review" && <div className="screen with-nav review-screen">
        <Header title="家庭日历" /><span className="eyebrow">有记录的夜晚和已实现的期待会留在这里</span><div className="review-insight"><AppIcon name="quiet" /><div><small>本周执行复盘</small><strong>{weeklyLessPromptNights ? `有${weeklyLessPromptNights}晚，共同执行更顺畅` : weeklyReflections.length ? `已记录${weeklyReflections.length}晚执行感受` : weeklyNights ? "收尾时可以记录共同执行感受" : "先完成一晚真实计划"}</strong><p>{weeklyAdjustments ? `你们主动调整了${weeklyAdjustments}次，调整也是执行计划的一部分。` : weeklyNights ? "这里关注开始、完成和调整是否顺畅。" : "有记录后再复盘，不要求连续使用。"}</p></div></div><div className="month-nav"><button onClick={() => shiftMonth(-1)} aria-label="上个月"><Chevron direction="left" /></button><h1>{calendarYear}年{calendarMonth + 1}月</h1><button onClick={() => shiftMonth(1)} aria-label="下个月"><Chevron direction="right" /></button></div>
