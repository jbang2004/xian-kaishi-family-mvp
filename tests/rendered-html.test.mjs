...
test("contains the complete 先开始 product shell", async () => {
  ...
  assert.doesNotMatch(app, /<Header[^>]*\bback=/);
  assert.doesNotMatch(app, /backLabel=/);
  ...
  assert.match(app, /<Header home=\{goHome\} title="共同确认"/);
  assert.match(app, /className="flow-return-action" onClick=\{leaveDualStart\}>返回共同确认/);
  assert.match(app, /className="flow-return-action" onClick=\{\(\) => back\("plan"\)\}>返回今晚时间表/);
  assert.doesNotMatch(app, /className="running-support-strip"/);
  assert.doesNotMatch(app, /className="review-insight"/);
  assert.doesNotMatch(app, /className="metric-grid compact-metrics"/);
  assert.match(styles, /\.settings-group \{[\s\S]*?border: 0;[\s\S]*?background: transparent/);
  ...
  assert.match(app, /现在开始，整晚将比原定 \$\{plannedStartLabel\} 前移/);
  assert.match(app, /从现在一起开始<\/button>/);
  assert.match(app, /整晚会按原有时长顺延/);
  assert.match(app, /现在开始会整体前移/);
  ...
  assert.match(app, /onFocus=\{e => \{ beginStageTimeEdit\(stage\.id, "start"\); revealFormControl\(e\.currentTarget\); \}\}/);
  ...
  assert.match(app, /canStartRest && <button className="soft-button" onClick=\{startRestNow\}>休息 10 分钟<\/button>/);
  ...
  assert.match(app, /记录过的夜晚会留在这里/);
  assert.match(app, /过去的家庭时光仍然留在这里/);
  ...
  assert.doesNotMatch(app, /className="plan-editor-backdrop"/);
  assert.match(app, /const viewport = window\.visualViewport/);
  assert.match(app, /--app-viewport-height/);
  assert.match(app, /element\.scrollIntoView\(\{ block: "center", inline: "nearest"/);
  assert.match(styles, /\.stage-editor\.is-expanded \{[\s\S]*?position: relative;[\s\S]*?background: #fff;/);
  assert.doesNotMatch(styles, /\.timeline-disclosure\[open\] \.mini-timeline \{[\s\S]*?grid-auto-flow: column;/);
  ...
  assert.match(styles, /\/\* v14 — a template row owns one surface[\s\S]*?\.template-row > button\.template-row-main \{[^}]*display: grid;[^}]*border: 0;[^}]*border-radius: 0;[^}]*background: transparent;/);
  assert.match(styles, /\.template-row > button\.template-edit-button \{[^}]*border: 0;[^}]*border-left: 1px solid[^}]*border-radius: 0;[^}]*background: transparent;/);
